...
const ImageModal = () => {
  ...
  const handleUpload = async () => {
    ...
    console.log('Ukuran file (MB):', fileSizeInMb);
      ...
      const maxWidth = 600; // Lebar maksimum yang diinginkan
      const maxHeight = 600; // Tinggi maksimum yang diinginkan
    ...
    console.log('file type:', blob.type);
            ...
            if (response.ok) {
              ...
              setCurrentImage(data.url_image);
                  ...
                  {currentImage && currentImage.replace("http://103.127.132.64:3000", "http://api.figustack.com") ? <img className={`w-full h-full object-contain`}  src={`${currentImage.replace("http://103.127.132.64:3000", "http://api.figustack.com")}`}/> : null }
                    ...
                    <div className=" w-96 ">
                      ...
                      <button
                                className="bg-emerald-500 text-white active:bg-emerald-600 font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150"
                                type="button"
                                onClick={()=>handleImagePreview(currentUrl)}>
                                preview
                      </button>
